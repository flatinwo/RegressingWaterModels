...
data3 <- subset(data2a, Pressure > -125.) # to avoid crossing of lines
data4 <- subset(data2a, Pressure <= -125.) # to avoid crossing of lines
p <- ggplot(subset(data5, Source %in% c("Madrid+Princeton","Poole") & Temperature > 195.),aes(Temperature,Pressure,color=factor(Property),shape=factor(Source),alpha=factor(Model)))
...
p <- p + ylab("Pressure (MPa) ")+xlab("Temperature (K)") + scale_shape_discrete("Source") 
