...
class ThermoFit:
	def __init__(self,fn='./Data/compiledEOSAll', Tc=200, Pc=200, Rhoc=1.):
		sf = od.SpinodalFit(fn) #Maybe i don't need this as a member fxn
		...
		self.cS = np.array(sf.fitparams['P(T) parmas'])[::-1]
		self.Bg = np.array(5,5)
		self.Ws = np.array(5,5)
		self.Ls = np.array(5,5)
	...
	def getCp(self,T=200.,P=100.,computed=False):
		if not computed: self._compute(T,P)
	...
	def __compute(self, T=200,P=100,ind=0):
		...
		Pshat,S0 = self.criticalpt.reduceP(np.array([Ps,S0]))
