...
data2 <- subset(mydata, Source %in% c("TSEOS") & Temperature > -0.5)
...
p <- ggplot(subset(data5, Source %in% c("Madrid+Princeton","Poole") & Temperature > -0.5),aes(Temperature,Pressure,color=factor(Property),shape=factor(Source),alpha=factor(Model)))
