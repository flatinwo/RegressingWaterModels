library(ggplot2)
mydata <- read.table('RescaledST2.dat',header=TRUE)
theme_set(theme_gray(base_size = 24))
#tiff("AllDataPhaseDiagram.tiff",width=12,height=8,units='in',res=300)
pdf("ST2Rescaled.pdf")#,width=12,height=8)#,units='in',res=300)
...
p <- ggplot(subset(mydata, Source %in% c("Poole")),aes(Temperature,Pressure,color=factor(Property),shape=factor(Source)))
